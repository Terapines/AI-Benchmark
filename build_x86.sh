...
export PATH=/home/leguang.liu/work/mlir/triton/zcc/zcc-pro-u22_omp/bin:$PATH
source ~/work/mlir/triton/llvm-project/build/env.sh
...
# ABI=lp64d
GCC_X86="g++ -O3 -static"
# GCC="riscv64-unknown-linux-gnu-g++ -march=${ARCH} -mabi=${ABI} -O3 -static"
# ZCC="z++ -fno-lto --target=riscv64-unknown-linux-gnu -march=${ARCH} -mabi=${ABI} -O3 -static"
ZCC_X86="z++ -fno-lto -O3 -static"
AR="llvm-ar"
# OBJDUMP="llvm-objdump"

# Python virtual environment for triton kernel compilation
PYC="python"
TRITON_PLUGIN_DIRS=/home/leguang.liu/work/mlir/triton/triton-cpu
...
build_c_kernel_lib() {
  for kernel in ${C_KERNELS[@]}; do
    ...
    echo ${kernel}
    ${COMPILER} -fPIC -I ${DIR}/include -c ${kernel} ${FOMP} -o ${OBJ_DIR}/${name}.o
  done
...
build_triton_kernel_lib() {
  ...
  for kernel in ${TRITON_KERNELS[@]}; do
    ...
    for kernel_ir in ${KERNEL_AUX_FILE_DIR}/*.llir; do
      ...
      echo ${kernel_ir}
      llc  ${kernel_ir}  -o ${KERNEL_AUX_FILE_DIR}/${kernel_name}.s
      gcc -c -o ${OBJ_DIR}/${kernel_name}.o ${KERNEL_AUX_FILE_DIR}/${kernel_name}.s -g
      ...
      # ${ZCC} -S -x ir ${kernel_ir} -mllvm  -o ${KERNEL_AUX_FILE_DIR}/${kernel_name}.s
    ...
    for kernel_launcher in ${KERNEL_AUX_FILE_DIR}/*.cpp; do
      ...
      ${GCC_X86} -I ${DIR}/include -I ${KERNEL_LAUNCHER_INCLUDE_DIR} -c ${kernel_launcher} ${FOMP} -o ${OBJ_DIR}/${launcher_name}.o
    done
...
build_driver(){
  ...
  for main in ${DRIVERS[@]}; do
    ...
    ${COMPILER} ${main} -I ${DIR}/include -I ${KERNEL_LAUNCHER_INCLUDE_DIR} -L ${LIB_DIR} ${FOMP} -lkernel -lsupport -latomic -std=c++17 -D${KERNEL_ENABLE} -fPIC -o ${KERNEL_BIN_DIR}/${name}.elf
...
cp run.sh ./build/
sed -i 's/triton gcc zcc/triton gcc/g' ./build/run.sh
