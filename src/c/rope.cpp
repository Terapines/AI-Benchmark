...
void rope(float *t, float *freqs_cos, float *freqs_sin, float *output,
  ...
  for (int seq = 0; seq < seq_len; ++seq) {
    for (int batch = 0; batch < batch_num; ++batch) {
      for (int head = 0; head < head_num; ++head) {
        float *t_1 = &t[(seq * batch_num * head_num * head_dim) +
                        (batch * head_num * head_dim) + (head * head_dim)];
        float *t_2 = &t_1[half_dim];

        float *out_1 =
            &output[(seq * batch_num * head_num * head_dim) +
                    (batch * head_num * head_dim) + (head * head_dim)];
